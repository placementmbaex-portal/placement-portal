...
import { permanentlyDeleteCompany, permanentlyDeleteJob, type PermanentDeleteState } from "./actions";
...
export function PermanentDeleteModal({
  ...
  const action = kind === "company" ? permanentlyDeleteCompany : permanentlyDeleteJob;
  const [state, formAction, pending] = useActionState(
    action.bind(null, id, name),
    initialState,
  );

  const canDelete = typedName === name;
      ...
      >
        <div className="px-6 pt-5.5">
          ...
          <p className="mt-2.5 text-[14px] leading-[1.6] text-ink">
            This removes the {kind === "company" ? "company" : "role"} from the database
            entirely. It has no applications on record, so nothing else is affected — but
            this itself cannot be undone.
          </p>
